...
**Где мы сейчас:** Фаза 2 — реализованы 4 сервиса: account-service, product-service, order-service, notification-service.
...
Order-service обращается к product-service по HTTP (`ProductClient`). Таймауты настраиваются через `app.http.client.connect-timeout` / `app.http.client.read-timeout`.
...
- Пессимистичная блокировка: `@Lock(PESSIMISTIC_WRITE)` в JPA-запросах или нативный `FOR UPDATE SKIP LOCKED`
...
- Асинхронные проверки: `Awaitility` (есть в зависимостях order-service)
